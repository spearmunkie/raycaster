...
var wallSlice : array 1 .. 4, 0 .. 63 of int
...
proc Raycast (angle, x, y : real)
	...
	if dist (1) < dist (2) and ang not= 359 and ang not= 179 then
	    ...
	    portionToDraw := Pic.Scale (portionToDraw, 5, drawHeight)
...
loop
    ...
    if key ('w') and not collide (human.x + cosd (human.angle) * 30, human.y + sind (human.angle) * 30, 25) then
	human.x += cosd (human.angle) * 30
	human.y += sind (human.angle) * 30
    end if
    if key ('s') and not collide (human.x - cosd (human.angle) * 15, human.y - sind (human.angle) * 30, 25) then
	human.x -= cosd (human.angle) * 30
	human.y -= sind (human.angle) * 30
    end if
