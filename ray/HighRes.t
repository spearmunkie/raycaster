...
wallSlice (1, 1) := Pic.FileNew ("Bricks.bmp")
...
proc Raycast (angle, x, y : real)
	...
	if dist (1) < dist (2) and round (ang) not= 359 and round (ang) not= 179 then
...
loop
    ...
    if key ('w') and not collide (human.x + cosd (human.angle) * 70, human.y + sind (human.angle) * 70, 25) then
	human.x += cosd (human.angle) * 70
	human.y += sind (human.angle) * 70
    end if
    if key ('s') and not collide (human.x - cosd (human.angle) * 70, human.y - sind (human.angle) * 70, 25) then
	human.x -= cosd (human.angle) * 70
	human.y -= sind (human.angle) * 70
    end if
